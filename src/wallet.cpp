...
// RISE: optional setting to unlock wallet for block minting only;
...
// RISE: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // RISE: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // RISE: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    else
                    {
                        ...
                        if (!GetBoolArg("-avatar")) // RISE: not avatar mode
                        {
...
// RISE: create coin stake transaction
...
// RISE: check 'spent' consistency between wallet and txindex
// RISE: fix wallet spent state according to txindex
...
// RISE: disable transaction (only for coinstake)
