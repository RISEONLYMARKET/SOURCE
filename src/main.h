...
static const int STAKE_MAX_AGE = 60 * 60 * 24 * 400;
...
public:
    ...
    {
        // RISE: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CTxDB& txdb, uint64& nCoinAge) const;  // RISE: get transaction coin age
...
public:
    ...
    // RISE: block signature - signed by coin base txout[0]'s owner
    ...
    // RISE: two types of block: proof-of-work or proof-of-stake
    ...
    // RISE: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64& nCoinAge) const; // RISE: calculate total coin age spent in block
    bool SignBlock(const CKeyStore& keystore);
    bool CheckBlockSignature() const;
    unsigned int GetStakeEntropyBit() const; // RISE: entropy bit for stake modifier if chosen by modifier
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    CBigNum bnChainTrust; // RISE: trust score of block chain
    int nHeight;
    int64 nMint;
    int64 nMoneySupply;

    unsigned int nFlags;  // RISE: block index flags
    enum  
    {
